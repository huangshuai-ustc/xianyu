...
url = 'https://pa.sevenoaks.gov.uk/online-applications/search.do?action=advanced'

data, summaries, statuses, addresses,dates_validated, refs, applicationTypes, fromDates, toDates, urls = [],[],[],[],[],[],[],[],[],[]
decisions = []

def get_data(Application_Type, Description_Keyword):
    ...
    for i in range(len(start_time)):
        ...
        driver.find_element(By.CSS_SELECTOR, '#description').send_keys(Description_Keyword)
        ...
        def get_a_page_data():
            ...
            for _ in all_datas:
                ...
                address = all_data[1]
                Status = all_data[2].split(' | ')[3].split(': ')[-1]
                date_validated = all_data[2].split(' | ')[2].split(': ')[-1]
    ...
    df.to_excel('{}_{}.xlsx'.format(Application_Type, Description_Keyword), index=False, header=header)


if __name__ == '__main__':
    Application_Type = 'Full Application'
    Description_Keyword = 'use as'
    get_data(Application_Type, Description_Keyword)
