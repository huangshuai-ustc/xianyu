...
url = 'https://www.lewes-eastbourne.gov.uk/article/2101/Advanced-Search'
...
def get_data(Application_Type):
    ...
    for i in range(len(start_time)):
        ...
        # s1 = Select(driver.find_element(By.CSS_SELECTOR, '#view221'))  # 这里是Application Type下拉单选框
        # s1.select_by_visible_text(Application_Type)
        driver.find_element(By.CSS_SELECTOR, '#view221').send_keys(Application_Type)
        # driver.find_element(By.CSS_SELECTOR, '#txtProposal').send_keys('use as')
        # driver.find_element(By.CSS_SELECTOR, '#rbRange').click()
        start_date = driver.find_element(By.CSS_SELECTOR, '#view228')  # 开始日期输入框
        start_date.clear()  # 清空输入框内容
        start = start_time[i]
        start_date.send_keys(start)  # 发送开始时间
        end_date = driver.find_element(By.CSS_SELECTOR, '#view229')  # 结束日期输入框
        end_date.clear()  # 清空输入框内容
        end = end_time[i]
        end_date.send_keys(end)  # 发送结束时间
        driver.find_element(By.CSS_SELECTOR, '#advancedSearch > div > button').click()  # 点击搜索
        time.sleep(10)
        ...
        def get_a_page_data():
            all_datas = driver.find_elements(By.CLASS_NAME, 'civicadetail')
            names = driver.find_elements(By.CSS_SELECTOR, '#planningResults > div > div > div:nth-child(1) > div:nth-child(2) > ul > li > div > div > div > div > div > div > div > div > div > a')
            for _ in range(int(len(all_datas)/3)):
                name = names[_].text
                print(name)
                link = driver.find_element(By.LINK_TEXT, name).get_attribute('href')
                address = all_datas[3*_+1].text
                ref = all_datas[3*_+2]
                date_validated = name.split(' ')[-1]
        ...
        while number != 0:
            ...
            if i == 0:
                get_a_page_data()
                try:
                    driver.find_element(By.CLASS_NAME, '#planningResults > div > div > div:nth-child(2) > div > div > div').click()
                    time.sleep(5)
                except:
                    number = 0
            elif i == 1:
                get_a_page_data()
                try:
                    driver.find_element(By.CLASS_NAME, '#planningResults > div > div > div:nth-child(2) > div > div > div:nth-child(2)').click()
                    time.sleep(5)
                except:
                    number = 0
            else:
                ...
                try:
                    driver.find_element(By.CLASS_NAME,
                                        '# planningResults > div > div > div:nth-child(2) > div > div > div:nth-child(3)').click()
                    time.sleep(5)
                except:
                    number = 0
            i += 1
