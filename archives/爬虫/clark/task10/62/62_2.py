...
url = 'https://southhams.planning-register.co.uk/Disclaimer?returnUrl=%2FSearch%2FAdvanced'
    ...
    for i in range(len(start_time)):
        ...
        if number != 0:
            for _ in range(int((number - 1) / 10)):
                if _ == 0:
                    get_a_page_data()
                    driver.find_elements(By.CSS_SELECTOR, '#results > div > div > div.pager > div > ul > li > a')[-1].click()
                else:
                    time.sleep(2)
                    get_a_page_data()
                    driver.find_elements(By.CSS_SELECTOR, '#results > div > div > div.pager > div > ul > li > a')[-1].click()
