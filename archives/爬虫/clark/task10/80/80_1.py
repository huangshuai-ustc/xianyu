...
url = 'https://planning.ashfield.gov.uk/planning-applications/search-applications/'
...
def get_data(Application_Type):
    ...
    for i in range(len(start_time)):
        ...
        driver.find_element(By.ID, 'view187').send_keys(Application_Type)
        # driver.find_element(By.CSS_SELECTOR, '#txtProposal').send_keys('use as')
        # driver.find_element(By.CSS_SELECTOR, '#rbRange').click()
        start_date = driver.find_element(By.ID, 'view193')  # 开始日期输入框
        start_date.clear()  # 清空输入框内容
        start = start_time[i]
        start_date.send_keys(start)  # 发送开始时间
        end_date = driver.find_element(By.ID, 'view194')  # 结束日期输入框
        end_date.clear()  # 清空输入框内容
        end = end_time[i]
        end_date.send_keys(end)  # 发送结束时间
        driver.find_element(By.CSS_SELECTOR, '#civica-planningsearchandview > div > div.civica-search-and-view-page.civica-search-and-results-page > div:nth-child(1) > div > button').click()  # 点击搜索
        # time.sleep(5)
        ...
        def get_a_page_data():
            names = driver.find_elements(By.CSS_SELECTOR, '#civica-result13 > div > div > div:nth-child(1) > div:nth-child(2) > ul > li > div > div > div > div > div > div > div > div > div > a')
            ...
            addresss = driver.find_elements(By.CSS_SELECTOR, '#civica-result13 > div > div > div:nth-child(1) > div:nth-child(2) > ul > li > div > div > div > div > div > div > div > div > div > div:nth-child(2) > div')
            ...
            refss = driver.find_elements(By.CSS_SELECTOR, '#civica-result13 > div > div > div:nth-child(1) > div:nth-child(2) > ul > li > div > div > div > div > div > div > div > div > div > div:nth-child(4) > div')
            for _ in range(len(names)):
                ...
                appnums.append(name.split(' - ')[0].replace('Planning Application ', ''))
                refs.append(ref)
                # summaries.append(summary)
                addresses.append(address)
                # statuses.append(Status)
                dates_validated.append(name.split(' - ')[1].replace('Valid From ', ''))
                decisions.append('')
...
if __name__ == '__main__':
    Application_Types = 'change of use OR conversion OR use as'
    for i in Application_Types.split(' OR '):
        get_data(Application_Types)
