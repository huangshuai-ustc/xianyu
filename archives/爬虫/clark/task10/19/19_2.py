...
def get_data(Application_Type, keyword):
    ...
    for i in range(len(start_time)):
        ...
        driver.find_element(By.CSS_SELECTOR, '#ctl00_ContentPlaceHolder1_txtPlanningKeywords').send_keys(keyword)
    ...
    df.to_excel('./{}_{}.xlsx'.format(Application_Type.replace('/', ''), keyword), index=False, header=header)


if __name__ == '__main__':
    Application_Type = 'Full Planning'
    Keywords = 'Change of use OR conversion OR use as'
    for i in Keywords.split(' OR '):
        get_data(Application_Type, i)
