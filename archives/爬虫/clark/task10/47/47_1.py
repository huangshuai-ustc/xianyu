...
url = 'https://planning.northdevon.gov.uk/Search/Advanced'
...
def get_data(Application_Type):
    ...
    for i in range(len(start_time)):
        ...
        start_date = driver.find_element(By.CSS_SELECTOR, '#DateDeterminedFrom')  # 开始日期输入框
        start_date.clear()  # 清空输入框内容
        start = start_time[i]
        start_date.send_keys(start)  # 发送开始时间
        end_date = driver.find_element(By.CSS_SELECTOR, '#DateDeterminedTo')  # 结束日期输入框
        ...
        try:
            number = driver.find_element(By.CSS_SELECTOR, '#results > p:nth-child(3)').text
        ...
        def get_a_page_data():
            # time.sleep(10)
            names = driver.find_elements(By.CSS_SELECTOR, '#results > div:nth-child(5) > table > tbody > tr > td:nth-child(2)')
            statuss = driver.find_elements(By.CSS_SELECTOR, '#results > div:nth-child(5) > table > tbody > tr > td:nth-child(5)')
            addresss = driver.find_elements(By.CSS_SELECTOR, '#results > div:nth-child(5) > table > tbody > tr > td:nth-child(3)')
            # decisionss = driver.find_elements(By.CSS_SELECTOR, '#application_results_table > tbody > tr > td:nth-child(8)')
            # refss = driver.find_elements(By.CSS_SELECTOR, '#results > div > div > div.results > div > table > tbody > tr:nth-child(1) > td:nth-child(3)')
            date_validates = driver.find_elements(By.CSS_SELECTOR, '#results > div:nth-child(5) > table > tbody > tr > td:nth-child(4)')
            for _ in range(len(names)):
                ...
                # ref = refss[_].text
                date_validated = date_validates[_].text
        ...
        if number != 0:
            for _ in range(int((number - 1))):
                if _ == 0:
                    get_a_page_data()
                    driver.find_elements(By.CSS_SELECTOR, '#resultsPager > nav > ul > li > a')[-1].click()
                else:
                    time.sleep(2)
                    get_a_page_data()
                    driver.find_elements(By.CSS_SELECTOR, '#resultsPager > nav > ul > li > a')[-1].click()
...
if __name__ == '__main__':
    Application_Types = 'Prior Approval Class G OR Prior Approval Class M OR Prior Approval Class M OR Prior Approval Class MA OR Prior Approval Class O OR Prior Approval Class P OR Prior Approval Class PA OR Prior Approval Class Q'
