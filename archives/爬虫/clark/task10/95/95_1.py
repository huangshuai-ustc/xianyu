...
url = 'https://online.warrington.gov.uk/planning/index.html'
...
def get_data(Application_Type):
    ...
    for i in range(len(start_time)):
        driver.get(url)
        driver.find_element(By.CSS_SELECTOR, '#content > div > div > div > div.row.homepage > div > div:nth-child(2) > div.col-md-8 > div:nth-child(1) > a').click()
        time.sleep(3)
        try:
        ...
        s1 = Select(driver.find_element(By.ID, 'ps_development_code_id'))  # 这里是Application Type下拉单选框
        ...
        start_date = driver.find_element(By.CSS_SELECTOR, '#valid_date_from')  # 开始日期输入框
        start_date.clear()  # 清空输入框内容
        start = start_time[i]
        start_date.send_keys(start)  # 发送开始时间
        end_date = driver.find_element(By.CSS_SELECTOR, '#valid_date_to')  # 结束日期输入框
        end_date.clear()  # 清空输入框内容
        end = end_time[i]
        end_date.send_keys(end)  # 发送结束时间
        driver.find_element(By.CLASS_NAME, 'btn btn-success').click()  # 点击搜索
        # time.sleep(5)
        ...
        def get_a_page_data():
            names = driver.find_elements(By.CSS_SELECTOR, '#application_results_table > tbody > tr > td:nth-child(1)')
            ...
            addresss = driver.find_elements(By.CSS_SELECTOR, '#application_results_table > tbody > tr > td:nth-child(3)')
            decisionss = driver.find_elements(By.CSS_SELECTOR, '#application_results_table > tbody > tr > td:nth-child(8)')
            refss = driver.find_elements(By.CSS_SELECTOR, '#application_results_table > tbody > tr > td:nth-child(4)')
            ...
            for _ in range(len(names)):
                ...
                data_id = urlss[_].get_attribute('data-id')
                link = f'https://online.warrington.gov.uk/planning/index.html?fa=getApplication&id={data_id}'
...
if __name__ == '__main__':
    Application_Type = 'Q20 - Other: Change of use'
