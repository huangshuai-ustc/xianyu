    ...
    try:
        ...
        try:
            net_profits = driver.find_element(By.XPATH,
                                              '//*[@id="cwzbTable"]/div[1]/div[1]/div[4]/div/table[2]/tbody/tr[2]').text
        except:
            net_profits = '无法查找到该字段信息'
        try:
            net_profit5 = []
            for i in net_profits.split(' ')[:5]:
                print(i)
                if i[-1] == '亿':
                    net_profit5.append(str(float(i.split('亿')[0])*100000000))
                elif i[-1] == '万':
                    net_profit5.append(str(float(i.split('万')[0]) * 10000))
                else:
                    net_profit5.append(str(i))
        except:
            net_profit5 = ['无法查找到该字段信息', '无法查找到该字段信息', '无法查找到该字段信息', '无法查找到该字段信息', '无法查找到该字段信息']
        try:
            gross_revenues = driver.find_element(By.XPATH,
                                                 '//*[@id="cwzbTable"]/div[1]/div[1]/div[4]/div/table[2]/tbody/tr[6]').text
        except:
            gross_revenues = '无法查找到该字段信息'
        try:
            gross_revenue5 = []
            for i in gross_revenues.split(' ')[:5]:
                if i[-1] == '亿':
                    gross_revenue5.append(str(float(i.split('亿')[0])*100000000))
                elif i[-1] == '万':
                    gross_revenue5.append(str(float(i.split('万')[0]) * 10000))
                else:
                    gross_revenue5.append(str(i))
        except:
        ...
        return [i + 1, name, code, industry, employee, product_name, company_profile,
                where, main, ctrl, times,
                net_profit5[0],
                net_profit5[1], net_profit5[2], net_profit5[3],
                net_profit5[4], gross_revenue5[0], gross_revenue5[1], gross_revenue5[2],
                gross_revenue5[3],
                gross_revenue5[4], asset_liability_ratio5[0], asset_liability_ratio5[1],
                asset_liability_ratio5[2],
...
f = open('y.csv', 'a+b')
df.to_csv(f, header=['序号', '股票名称', '股票代码', '行业', '员工人数', '产品', '简介', '办公地址', '主营业务',
