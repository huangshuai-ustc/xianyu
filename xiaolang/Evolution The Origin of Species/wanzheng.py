...
# used to index an animal_template
ANIMAL_TRAITS = 0
ANIMAL_FOOD_COUNT = 1
...
def can_predator_attack_prey(predator_animal, prey_animal):
    ...
    if (TRAIT_CAMOUFLAGE in prey_traits
            and TRAIT_SHARP_VISION not in predator_traits):
        return False
    ...
    for i, player in enumerate(players):
        ...
        for j, predator in enumerate(animals_predator):
            ...
            for k, other_player in enumerate(players):
                ...
                for m, prey in enumerate(animals_q):
                    prey_key = (k, m)
...
def simulate_optimal_feeding_phase_gameplay(players, focus_player_index, red_tokens_available):
    ...
    for i, player in enumerate(players):
        ...
        for j, animal in enumerate(animals):
            ...
            if not animal[2]:
                ...
                # red_used is needing
                _, red_used_count, _ = feed_animal_red_token(sim_animals, j)
                # Check if the red_used is the best
                if red_used_count > best:
                    best = red_used_count
        # Store the best move
        ...
        if red <= 0:
            ...
            for k, p in enumerate(players):
                if k == focus_player_index:
                    ...
                    final_players.append(copy.deepcopy(p))
            ...
            total = 0.0
            for t, focus_animal in enumerate(focus_animals):
    ...
    best_scores, best_turns = best_result
    return best_scores, best_turns
